...
// ─── PERSISTENT STORAGE ───────────────────────────────────────────────────────
const DB = {
  load: async (key) => { try { const r = await window.storage.get(key,false); return r?JSON.parse(r.value):null; } catch { return null; } },
  save: async (key,val) => { try { await window.storage.set(key,JSON.stringify(val),false); } catch {} },
};
      ...
      <div style={{width:"100%",maxWidth:380}}>
        ...
        <Card>
          ...
          <Input type="password" value={pw} onChange={e=>setPw(e.target.value)}
            ...
            style={{marginBottom:14}} onKeyDown={e=>e.key==="Enter"&&setAuthed(true)}/>
          <Btn onClick={()=>setAuthed(true)} accent={C.o} full>AUTHENTICATE →</Btn>
          <div style={{fontSize:9,color:C.dim,textAlign:"center",marginTop:10}}>Demo: any password</div>
        </Card>
      </div>
    </div>
  );
